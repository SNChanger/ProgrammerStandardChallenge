...
pub struct AddUserInteractor {
    presenter: Box<dynn OutputBoundary<AddUserOutputMessage, AddUserError>>,
    add_user_repository: Box<dy AddUserPort>,
...
impl InputBoundary<AddUserInputMessage> for AddUserInteractor 
{
    fn execute(&mut self, message: AddUserInputMessage) {
        ...
        match result_user {
            ...
            Err(e) => {
                self.presenter.error(self.output_message.clone(), err);
                return;
